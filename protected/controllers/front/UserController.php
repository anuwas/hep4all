...
class UserController extends Controller
{
	...
	public function actionAddexercise()
	{
		...
		if(isset($_POST['Exercises']))
		{
			...
			$exercisetitle=$_POST['Exercises']['exercise_title'];
			$this->sendMailToAdmin($userid,$exercisetitle,$model->picture_1,$model->picture_2,$model->video_1,$model->video_2);
		...
		$exercisetype=ExerciseTypeMaster::model()->findAll(array('order'=>'exercise_type_name'));
		$exercisecategory=ExerciseCategoryMaster::model()->findAll(array('order'=>'exercise_category_name'));
		$exercisesubcategory=ExerciseSubCategoryMaster::model()->findAll(array('order'=>'exercise_sub_category_name'));
		$position=PositionMaster::model()->findAll(array('order'=>'position_master_name'));
	...
	public function actionSetprintexercisefromtemplate()
	{
		$userid=Yii::app()->session['user_id'];
		$printmasterid=$_REQUEST['printmasterid'];
		PrintExercise::model()->deleteAllByAttributes(array('user_id'=>$userid));
		$printdetail=PrintDetail::model()->findAllByAttributes(array('print_master_id'=>$printmasterid));
		foreach ($printdetail as $value) {
			$pritexercuse=new PrintExercise();
			$pritexercuse->user_id=$userid;
			$pritexercuse->exercise_id=$value->exercise_id;
			$pritexercuse->serial=$value->serial;
			$pritexercuse->photo_number=$value->photo_number;
			$pritexercuse->perform=$value->perform;
			$pritexercuse->times=$value->times;
			$pritexercuse->complete_set=$value->complete_set;
			$pritexercuse->reps=$value->reps;
			$pritexercuse->hold=$value->hold;
			$pritexercuse->routene_name=$value->routene_name;
			$pritexercuse->description=$value->description;
			$pritexercuse->save();
		}
	}

	public function actionSharedbyother()
	{
		$userid=Yii::app()->session['user_id'];
		$user=Users::model()->with('country_master','occupation_master','workSetting')->findByAttributes(array('user_id'=>$userid));
		//$templates=PrintMaster::model()->findAllByAttributes(array('user_id'=>$userid,'template_type'=>1));
		$templates=TemplateEmailHistory::model()->with('printMaster','user')->findAllByAttributes(array('to_email'=>$user->email,'show_other_wall'=>1));
		$this->render('sharedbuothers',array('model'=>$user,'templates'=>$templates));
	}
	
	public function actionViewemailtemplatehostory()
	{
		$userid=Yii::app()->session['user_id'];
		$user=Users::model()->with('country_master','occupation_master','workSetting')->findByAttributes(array('user_id'=>$userid));
		$printmasterid=$_REQUEST['id'];
		$templateemail=TemplateEmailHistory::model()->with('printMaster')->findAllByAttributes(array('print_master_id'=>$printmasterid));
		$this->render('favourite_tempalte_email',array('model'=>$user,'templates'=>$templateemail));
	}
	
	public function actionHepstatus()
	{
		$userid=Yii::app()->session['user_id'];
		$user=Users::model()->with('country_master','occupation_master','workSetting')->findByAttributes(array('user_id'=>$userid));
		$criteria = new CDbCriteria;
		$criteria->order = 'tehid DESC';
		$criteria->condition="t.user_id=$userid";
		$printexercise=TemplateEmailHistory::model()->with('printMaster')->findAll($criteria);
		$this->render('hepstatus',array('model'=>$user,'emailhistory'=>$printexercise));
	}
	...
	if(isset($_FILES["myfile"]))
{
	...
	$error =$_FILES["myfile"]["error"];
   {
	...
	protected function sendMailToAdmin($userid,$title,$image1,$image2,$video1,$video2)
	{
		$user=Users::model()->findByAttributes(array('user_id'=>$userid));
		//$to='todd@emr4all.com';	
		$to='toddj@me.com';
		$subject = "HEP4all - New Exercise Added";
		...
		<table>
			...
			<tr>
				<td>Exercise Title : '.$title.'</td>
			</tr>
			<tr>
				<td><img align="center" alt="" src="http://www.hep4all.com/upload/exercise/image/537x480/'.$image1.'"></td>
			</tr>';
			if($video1!='sample.mp4'){
				$message.='<tr>
					<td><a href="//player.vimeo.com/video/'.$video1.'">Click here to see video 1</a></td>
				</tr>';
			}
			$message.='<tr>
				<td><img align="center" alt="" src="http://www.hep4all.com/upload/exercise/image/537x480/'.$image2.'"></td>
			</tr>';
		
		if($video2!='sample.mp4'){
		$message.='<tr>
				<td><a href="//player.vimeo.com/video/'.$video2.'">Click here to see video 2</a></td>
			</tr>';
		}
