...
(function(b) {
	if (b.fn.ajaxForm == undefined) {
		b.getScript("http://localhost/upload/single/js/jquery.form.js")
	}
	var a = {};
	...
	b.fn.uploadFile = function(t) {
		...
		function j() {
			if (r.afterUploadAll && !g) {
				...
				(function s() {
					if (m.sCounter != 0
							&& (m.sCounter + m.fCounter == m.tCounter)) {
						r.afterUploadAll(m);
						g = false
					} else {
		...
		function l(H, B, u) {
			for ( var C = 0; C < u.length; C++) {
				if (!c(B, H, u[C].name)) {
					if (H.showError) {
						b(
								"<div style='color:red;'><b>" + u[C].name
										+ "</b> " + H.extErrorStr
										+ H.allowedTypes + "</div>").appendTo(
								B.errorLog)
					}
					continue
				}
				...
				if (H.maxFileCount != -1 && B.selectedFiles >= H.maxFileCount) {
					if (H.showError) {
						b(
								"<div style='color:red;'><b>" + u[C].name
										+ "</b> " + H.maxFileCountErrorStr
										+ H.maxFileCount + "</div>").appendTo(
								B.errorLog)
					}
					continue
				}
				...
				E.filename.html(G);
				var v = b("<form style='display:block; position:absolute;left: 150px;' class='"
						+ B.formGroup
						+ "' method='"
						+ H.method
						+ "' action='"
						+ H.url + "' enctype='" + H.enctype + "'></form>");
				v.appendTo("body");
				var x = [];
				x.push(u[C].name);
				n(v, D, E, x, B);
		...
		function q(y, B, D, u) {
			...
			var w = b("<form method='" + D.method + "' action='" + D.url
					+ "' enctype='" + D.enctype + "'></form>");
			...
			if (D.multiple) {
				...
				v = "<input type='file' id='" + A + "' name='" + D.fileName
						+ "' multiple/>"
			}
			...
			z.change(function() {
				...
				if (this.files) {
					...
					if (!c(y, D, I)) {
						if (D.showError) {
							b(
									"<div style='color:red;'><b>" + I + "</b> "
											+ D.extErrorStr + D.allowedTypes
											+ "</div>").appendTo(y.errorLog)
						}
						return
					}
					F.push({
				...
				if (a.fileapi && a.formdata) {
					...
					for ( var H = 0; H < G.length; H++) {
						if (D.showFileCounter) {
							E += y.fileCounter + D.fileCounterStyle + G[H]
									+ "<br>"
						} else {
							E += G[H] + "<br>"
						}
					...
					if (D.maxFileCount != -1
							&& (y.selectedFiles + G.length) > D.maxFileCount) {
						if (D.showError) {
							b(
									"<div style='color:red;'><b>" + E + "</b> "
		...
		function p(v, u) {
			...
			this.filename = b("<div class='upload-filename'></div>").appendTo(
					this.statusbar);
			this.progressDiv = b("<div class='upload-progress'>").appendTo(
					this.statusbar).hide();
			this.progressbar = b(
					"<div class='upload-bar " + v.formGroup + "'></div>")
					.appendTo(this.progressDiv);
			this.abort = b(
					"<div class='upload-red " + v.formGroup + "'>" + u.abortStr
							+ "</div>").appendTo(this.statusbar).hide();
			this.cancel = b("<div class='upload-red'>" + u.cancelStr + "</div>")
					.appendTo(this.statusbar).hide();
			this.done = b("<div class='upload-green'>" + u.doneStr + "</div>")
					.appendTo(this.statusbar).hide();
			this.del = b("<div class='upload-red'>" + u.deletelStr + "</div>")
					.appendTo(this.statusbar).hide();
		...
		function n(z, y, u, w, A) {
			...
			var v = {
				...
				beforeSubmit : function(F, C, E) {
					...
					u.statusbar.append("<div style='color:red;'>"
							+ y.uploadErrorStr + "</div>");
					u.cancel.show();
					z.remove();
